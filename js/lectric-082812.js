...
// isWebkit = true;
  ...
  BaseSlider.prototype.update = function(opts) {
	...
	var isEnd = ( this.numItems == parseInt(this.pageNum) ? true : false  );
	var isBeginning = ( this.pageNum == 0 ? true : false  ) ;
	//console.log('update');
    var self = this;
    var after = function() {
      self.element.trigger('animationEnd.lectric');
		...
		if( self.looped ){
			//reset slider back to beginning or end positions
			//console.log('looped');
			self.looped = false;
			
			if( parseInt(self.pageNum) == self.numItems ){
				console.log('looping back to beginning now')
				// loop end back to beginning
				//self.pageNum = 0;
				console.log(self);
				self.element.find(self.element.itemSelector).filter(':first-child').add(self.element)
					.css({left: '0%', '-webkit-transition': 'none 0s'}).delay(1).queue(function (){
				...
				//alert('dun');
				self.position.x = 0;
				//self.element.css({'-webkit-transition': ''});
			} else {
				//loop beginning back to end
				self.pageNum = 0;
				self.element.find(self.element.itemSelector).filter(':first-child').add(self.element).css({'-webkit-transition': '0s none', left: '0%' });
    ...
    if (options.animate) {
		if(isBeginning && options.direction < 0 ){
			//move first slide to end;
			this.looped = true;
			console.log('loop first to end');
			
			var endElement = this.element.find(this.element.itemSelector).filter(':last-child');
			endElement.css({left: "-" + this.pageNum + '00%'});
		
		} else if ( isEnd && options.direction > 0 ) {
			// move last slide before first slide
			console.log('loop last to beginning');
			this.looped = true;
			var beginningElement = this.element.find(this.element.itemSelector).filter(':first-child');
			beginningElement.css({left: this.numItems + '00%'});
