...
(function(window, $) {
  ...
  BaseSlider.prototype.holdPage = function() {
	console.log(this.pageNum);
	this.position.x = (-this.pageNum*$(window).width());
      ...
      if (this.moved) {
        ...
        if (this.opts.tossing) {
          ...
          if (dx > this.opts.slideThreshold && page > 0){
			...
			if (page < (itemCt-1)){
				this.position.x = -page * width;
			} else{
				//keeps from swiping to non-existant slide at end 
				this.position.x = -(itemCt-1) * width;
			}
          }
